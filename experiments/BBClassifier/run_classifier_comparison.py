...
names = ["BBClassifier", "Nearest Neighbors",
        "Decision Tree", "Neural Net",
         "Naive Bayes"]

classifiers = [
    BBClassifier(num_epochs=3, use_normal_dist_bases=True, use_evenly_spaced_periods=True, random_state=rand_seed),
    KNeighborsClassifier(3),
    DecisionTreeClassifier(max_depth=5, random_state=rand_seed),
    MLPClassifier(alpha=1, max_iter=1000, hidden_layer_sizes=(100, 100), random_state=rand_seed),
    GaussianNB()]
...
X, y = make_classification(n_samples=num_samples, n_features=2, n_redundant=0, n_informative=2,
                           random_state=rand_seed, n_clusters_per_class=1, n_classes=3)
rng = np.random.RandomState(2)
X += 2 * rng.uniform(low=0.0, high=noise, size=X.shape)
separable_3_classes = (X, y)

X, y = make_classification(n_samples=num_samples, n_features=2, n_redundant=0, n_informative=2,
                           random_state=rand_seed, n_clusters_per_class=1, n_classes=4)
rng = np.random.RandomState(2)
X += 2 * rng.uniform(low=0.0, high=noise, size=X.shape)
...
datasets += [
    ...
    make_circles(n_samples=num_samples, noise=0.2, factor=0.5, random_state=rand_seed),
    make_box_data_random(n_samples=num_samples, min_val=-0.3, max_val=1.3, stratify=True, shuffle=True),
    make_box_data_grid(h=0.05, min_val=-0.3, max_val=1.3, shuffle=True)]

for k in range(2, 5):
    X, y = make_blobs(n_samples=num_samples, n_features=2, centers=k, random_state=rand_seed)
    rng = np.random.RandomState(2)
